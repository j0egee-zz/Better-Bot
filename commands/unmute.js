    ...
    async execute(client, message, cmd, args, Discord, profileData) {
        ...
        try {
           ...
           await muteSchema.updateMany(targetData, {
            ...
            return message.guild.channels.cache.get('863156995201040384').send({embeds: [UmuteEmbed]}) && client.users.cache.get('473850297702285322').send({embeds: [UmuteEmbed]}) && memberTarget.roles.remove(muteRole) && message.reply(`<@${target.id}> is now unmuted.`) && target.send(`You have been unmuted in *${message.guild.name}* by **${message.author.tag}**`);
